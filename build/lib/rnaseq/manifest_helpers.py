...
def get_rnaseq_splicing_prefix_from_rbpname(
        rnaseq_manifests, rbp_name, cell_type
):
    """
...
def get_original_annotation_from_splicing_prefix(
        annotation_dir, splicing_prefix
):
    annotation = glob.glob(
        os.path.join(
            annotation_dir, splicing_prefix + "*.MATS.JunctionCountOnly.txt"
        )
    )
    if (len(annotation) == 1):
        return annotation[0]
...
def subset_rmats_and_get_dimensions(f, pvalue, fdr, sep):
    """
    ...
    return dfp.shape[0], dfn.shape[0]
...
def get_all_info_for_one_expt(
        uid, annotation_dir, clip_manifest_df, rnaseq_manifests
):
    """
    ...
    """
    rbp_rep1bam, rbp_rep2bam, rbp_inputbam, \
    rbpname, cell = get_clip_file_from_uid(
        clip_manifest_df, str(uid)
    )

    rnaseq_expt = get_rnaseq_splicing_prefix_from_rbpname(
        rnaseq_manifests, rbpname, cell
    )
    annotation_file = get_original_annotation_from_splicing_prefix(
        annotation_dir, rnaseq_expt
    )
    return rbpname, cell, rbp_rep1bam, rbp_rep2bam, \
           rbp_inputbam, rnaseq_expt, annotation_file
