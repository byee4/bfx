...
# my $manifest_fi = "/home/bay001/projects/codebase/bfx/from_eric/rna_seq_manifest.txt.2"; # truncated manifest
        ...
        for my $chr (keys %exon_triplets) {
            for my $str ("+","-") {
                for my $triplet (keys %{$exon_triplets{$chr}{$str}}) {
                    my $psi = sprintf('0');
                    # my ($jxns,$id) = split(/\:/,$triplet); # unused
                    my $jxns = $triplet; # exon triplet junction annotation
                    my $id = join("|",@{$exon_triplets{$chr}{$str}{$triplet}}); # genes?
                    ...
                    if ($exclusion_n + $inclusion_n >= 0) {
                        ...
                        for my $exon_triplet (@{$triplet2exons{$chr.":".$str.":".$triplet}}) {
                            ...
                            print $fh "${label}\t${psi}\t${chr}:${str}:${triplet}\t${up_ex}|${se_ex}|${dn_ex}\n";
                        }
                    }
                }
            }
